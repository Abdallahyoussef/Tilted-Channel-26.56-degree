...
for i=1:lx
    if i==1
        ...
        Kx(i:i+length(Kx_cave)-1,i)=Kx_cave;
    end
end
Kx=Kx';Kx=Kx(:);
Ky=Kx;
Theta=pi/6.7759; % inclination angel of channel by radian
...
startx=[(dx/2)*ones(1,ly/5) 0.5*dx:2*dx:dx*(lx-0.5)]; starty=[dy/2:5*dy:dy*(ly-0.5) linspace(dy/2,7.5,lx/2)]; hlines=streamline(x,y,vec2mat(Fx,lx),vec2mat(Fy,lx),startx,starty);
set(hlines,'linewidth',1,'color','b')
title('Streamline Map DMEPD')
